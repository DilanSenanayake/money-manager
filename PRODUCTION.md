...
| `NEXT_PUBLIC_GA_MEASUREMENT_ID` | Optional | GA4 Measurement ID. Defaults to `G-YH8936B98F` (smoneymanager.com web stream). Override on Vercel only if the stream changes. |
...
- Web traffic: Vercel Web Analytics (enable in the project) plus GA4 (`G-YH8936B98F`). Confirm hits in GA4 **Realtime** after deploy. Events (`login`, `sign_up`, `add_transaction`, CTA clicks) never include amounts, merchants, or emails.
